...
txt_box1 = ScrolledText(fr3, width=10, height=20, bg='light grey')
...
txt_box3 = ScrolledText(fr4, width=10, height=20, bg='light grey')
txt_box3.grid(row=2, column=1,pady=2,padx=2)

#LED
cl1 = Canvas(fr1, width=50, height=50, bg='white')
cl1.grid(row=0, column=2, pady=2, padx=2)
cl2 = Canvas(fr1, width=50, height=50, bg='white')
cl2.grid(row=0, column=3, pady=2, padx=2)
cl3 = Canvas(fr1, width=50, height=50, bg='white')
cl3.grid(row=0, column=4, pady=2, padx=2)
cl4 = Canvas(fr1, width=50, height=50, bg='white')
...
lb6 = Label (fr10, width=18, height=2, text='RASPI 1', font=("Calibri",15))
...
#garis garis
for i in range (11):
    cv1.create_text(35,380-40*i, text=i*75)
...
#============================================================================
btn = [24,18]  # Ubah pin sesuai dengan koneksi Anda
led = [19,16,26,20]
led2 = [[19,16],[26,20]]
...
alamat1=('192.168.1.67',8080) #alamat pakai local address, pengirim dan penerima dalam 1 komputer

penerima = socket.socket(socket.AF_INET,socket.SOCK_DGRAM)
alamat2=('192.168.1.64',8000) #alamat pakai local address, pengirim dan penerima dalam 1 komputer
##penerima.bind(alamat2)
def loop1():
    x=ser.readline()
    y=x.decode().rstrip()
    global v
...
def loop2():
    if loop_active==True:
        pengirim.sendto(v.encode(),alamat2)
